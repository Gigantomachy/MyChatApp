#!/usr/bin/env bash
# Bring the app cluster up for a session. Network is a no-op once it exists.
# The Cassandra DB is long-lived: provision it once with ./db.sh, it survives ./down.sh.
set -euo pipefail
